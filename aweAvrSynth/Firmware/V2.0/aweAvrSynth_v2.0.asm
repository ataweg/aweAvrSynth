\\Mybookworld\Axel\__GitHub__\aweAvrSynth\aweAvrSynth\Firmware\V2.0\aweAvrSynth.hex;-------------------------------------------------------------------------------------------------------------------
;
; aweAvrSynth
; Axel Werner
;
...
; Changelog
;
; 2011.11.25	AWe	assign ENV to DCA and ENV2 to DCF (swapped them)
;			rename envelop parameter DCF_<param>, DCA_<param> to <param> and <param>2
;			rename ENV2_<name> to ENV_<name>2
;			add MEEBLIP_ORIGINAL_CODE to control the usage of the original code (not fully implemented yet)
; 2011.11.06	AWe	move LED from Atmega pin PD1 to PB6
; 2011.11.04	AWe	change control range of VCFENVMOD from 0..255 to -128..127
; 2011.11.03	AWe	control PWM_WIDTH by midi value and if selected with sweep from LFO2
; 2011.11.02	AWe	improve noise generator, use 24 bit LFSR with taps at:
;			b24, b23, b22, b17. This is bit 7, 6, 5 and 0 of the msb.
; 2011.10.27	AWe	LFO2 expanded to have the same features as LFO
; 2011.10.26	AWe	start version 2.0 firmware
;			add second ADSR for VCA
;-------------------------------------------------------------------------------------------------------------------
; 2011.10.25	AWe	add support Midi-CC for PWMDEPTH, LFO2FREQ, VCFENVMOD
; 2011.10.25	AWe	add more headroom after the mixer and ringmodulator
; 2011.10.23	AWe	adjust headroom after ringmodulator
;			optimize function TAB_BYTE, value returned now in R16
;			correct sustain level after decay is done
; 2011.10.20	AWe	add Ringmodulator, use extended MODEFLAGS3,4
;			redesign DCOs, supported waveforms saw, tri, sin, squ, pwm
; 2011.10.17	AWe	optimize access to program memory, add more comments
; 2011.09.26	AWe	change OSC A/B mixing characteristics
;			used dead zone at the ends, search ";(2)"
; 2011.09.14	AWe	use change sawtooth wave to triangle  for OSC B
; 2011.09.07	AWe	comment out useless opcodes, search ";(1)"
; 2011.09.06	AWe	invert state of switches
; 2011.08.25	AWe	modify layout of switches, use include file "keyboard.inc"
; 2011.08.19	AWe	change ATmega32 pining swap DAC_RW/PD2 with DAC_AB/PD3
; 2011.03.04	AWe	code beautification, replace blanks with tabs (tab width is 8)
; 2011.03.04	AWe	use initial code from meeblib V1.05 for my meeblip clone (with hardware changes)
;
;-------------------------------------------------------------------------------------------------------------------

;-------------------------------------------------------------------------------------------------------------------
; my changes to original design
;	ATmega32 pining swap DAC_RW/PD2 with DAC_AB/PD3
;	support of RS232/serial interfac
;	support of Midi thru and Midi out
;	use a headphone amp with LM386
;
;-------------------------------------------------------------------------------------------------------------------
...
;-------------------------------------------------------------------------------------------------------------------
	...
	.equ	MEEBLIP_ORIGINAL_CODE	= 0
	.equ	MEEBLIP_SE_ENHANCEMENTS		= 1

.if MEEBLIP_ORIGINAL_CODE
	.equ	MEEBLIP_V1_27_HW	= 1	; 0: use my cloned&modified pcb version 1.27
	.equ	USE_MUL8X8S		= 1

	.equ	USE_ORIGINAL_MIXER	= 1	; 0: use alternative mixer
	.equ	USE_ORIGINAL_DCO_A	= 1
	.equ	USE_ORIGINAL_DCO_B	= 1
	.equ	USE_ORIGINAL_NOISEGEN	= 1
	.equ	USE_ORIGINAL_VCFENVMOD	= 1

	.equ	USE_OSC_B_SAWTOOTH	= 0	; 0: use triangle wavform for OSC B
	.equ	USE_RINGMODULATOR	= 0	;
...
.else
	...
	.equ	USE_ORIGINAL_DCO_A	= 0
	.equ	USE_ORIGINAL_DCO_B	= 0
	.equ	USE_ORIGINAL_NOISEGEN	= 0
	.equ	USE_ORIGINAL_VCFENVMOD  = 0
...
LFOFREQ:		.BYTE 1		; 0..255 read from ADC
LFOLEVEL:		.BYTE 1		; 0..255 read from ADC
PANEL_LFOLEVEL:		.BYTE 1		; 0..255 as read from the panel pot

DETUNEB_FRAC:		.BYTE 1		;\
DETUNEB_INTG:		.BYTE 1		;/ -128,000..+127,996 read from ADC
CUTOFF:			.BYTE 1		; 0..255 read from ADC
PORTAMENTO:		.BYTE 1		; 0..255 read from ADC

; DCF enevelop
ATTACKTIME2:		.BYTE 1		; 0..255 read from ADC
DECAYTIME2:		.BYTE 1		; 0..255 read from ADC
SUSTAINLEVEL2:		.BYTE 1		; 0..255 read from ADC
RELEASETIME2:		.BYTE 1		; 0..255 read from ADC
...
DCOA_LEVEL:		.BYTE 1		; read from ADC
DCOB_LEVEL:		.BYTE 1		; read from ADC
...
; fm
WAVEB:			.byte 1
FMDEPTH:		.byte 1		; 0..255 read from ADC
...
; filter
RESONANCE:		.byte 1	; 0..255 read from ADC
SCALED_RESONANCE:	.byte 1
b_L:			.byte 1
b_H:			.byte 1

.if USE_ORIGINAL_NOISEGEN == 0
LFSR_0:			.BYTE 1		;\
...
MIDICC:	.byte $80
	...
	.equ PWMDEPTH		= MIDICC + $0E	; Knob 1	14 ; currently not used
	.equ LFO2FREQ		= MIDICC + $0F	; Knob 2	15
	.equ VCFENVMOD		= MIDICC + $10	; Knob 3	16
;	.equ			= MIDICC + $11	; Knob 4	17
;	.equ			= MIDICC + $12	; Knob 5	18
;	.equ			= MIDICC + $13	; Knob 6	19
;	.equ			= MIDICC + $14	; Knob 7	20
;	.equ			= MIDICC + $15	; Knob 8	21
	.equ ATTACKTIME		= MIDICC + $16	; Slider 1	22 ; DCA envelop control
	.equ DECAYTIME		= MIDICC + $17	; Slider 2	23
	.equ SUSTAINLEVEL	= MIDICC + $18	; Slider 3	24
	.equ RELEASETIME	= MIDICC + $19	; Slider 4	25
;	.equ			= MIDICC + $1A	; Slider 5	26
;	.equ			= MIDICC + $1B	; Slider 6	27
;	.equ			= MIDICC + $1C	; Slider 7	28
;	.equ			= MIDICC + $1D	; Slider 8	29
...
;  AWe:  3429.17864 = 3429 + 0.17864 => 0D65 + round(0,17864*256) = 0D65,2E
;-------------------------------------------------------------------------------------------------------------------
...
TIMETORATE:
	.DW	65535		; 8.192 mS	fast lfo, attack/rise time
	.DW	50957		; 10.54 mS
	.DW	39621		; 13.55 mS
	.DW	30807		; 17.43 mS
	.DW	23953		; 22.41 mS
	...
	.DW	   44		; 12.09 S
	.DW	   35		; 15.55 S
	.DW	   27		; 20.00 S	; slow lfo, attack/rise time
...
;Calculate DCO A
; switches
;   MODEFLAGS1.1	b1 = wave A: 0=saw, 1=squ
;   MODEFLAGS1.6	b6 = (noise)
...
PULSE_ZERO:
	ldi	R17, 0
SAW_CHECK:
	sbrs	R30, 1			; b1 = wave A: 0=saw, 1=squ
...
;   MODEFLAGS1.6	b6 = (noise)
;
;   MODEFLAGS4.0	b0 = OSC A wave: 0=normal squ or SAW, 1=extended pwm or tri
;   MODEFLAGS4.1	b1 = OSC A wave: 0=normal, 1=sin

	lds	R31, MODEFLAGS4		;

	mov	r17, PHASEA_2		; sawtooth ramp for OSCA
	sbrc	r31, 1			; b1 = OSC A wave: 0=normal, 1=sin
	rjmp	DCOA_SIN		; do sin
	sbrs	r30, 1			; b1 = wave A: 0=saw, 1=squ
	rjmp	DCOA_TRI_SAW		; do tri or saw
; pwm or squ
	ldi	r22, 128		; pwm treshold for squ
	sbrc	r31, 0			; b0 = OSC A wave: 0=normal squ, 1=extended pwm
	lds	r22, PULSE_WIDTH	; do pwm
	cp	r17, r22
	brlo	PULSE_ZERO_A
	ldi	r17, 255		; sqr = 1
	rjmp	DCOA_DONE
...
DCOA_TRI_SAW:
	sbrs	r31, 0			; b0 = OSC A wave: 0=normal SAW, 1=extended tri
...
; triangle
	...
	lsl	r22
	rol	r17
	sbrs	r31, 1			; b1 = OSC A wave: 0=normal, 1=sin
...
;Calculate DCO B
; --------------------
.if USE_ORIGINAL_DCO_B
; switches
;   MODEFLAGS1.2	b2 = wave B: 0=SAW, 1=SQU
;   MODEFLAGS1.3	b3 = osc B: 0=off, 1=on

	mov	R16, PHASEB_2
	rol	R16			; R16.7 --> Cy
	sbc	R16, R16		; R16 = 0 or 255 (square wave)

.if USE_OSC_B_SAWTOOTH == 1
; sawtooth
	sbrs	R30, 2			; b2 = wave B: 0=SAW, 1=SQU
	mov	R16, PHASEB_2		; only when sawtooth
.else
; triangle
	sbrc	R30, 2			; b2 = wave B: 0=SAW, 1=SQU
	rjmp	CALC_DIST
	mov	R16, PHASEB_2
	sbrs	R16, 7
	com	R16
...
;   MODEFLAGS1.3	b3 = osc B: 0=off, 1=on
;
;   MODEFLAGS4.2	b2 = OSC B wave: 0=normal SQU or SAW, 1=extended pwm or tri
;   MODEFLAGS4.3	b3 = OSC B wave: 0=normal, 1=sin

	lds	R31, MODEFLAGS4		;

	mov	r16, PHASEB_2		; sawtooth ramp for OSCB
	sbrc	r31, 3			; b3 = OSC B wave: 0=normal, 1=sin
	rjmp	DCOB_SIN		; do sin
	sbrs	r30, 2			; b2 = wave A: 0=SAW, 1=SQU
	rjmp	DCOB_TRI_SAW		; do tri or saw
; pwm or squ
	ldi	r22, 128		; pwm treshold for squ
	sbrc	r31, 2			; b2 = OSC B wave: 0=normal SQU, 1=extended pwm
	lds	r22, PULSE_WIDTH	; do pwm
	cp	r16, r22
	brlo	PULSE_ZERO_B
	ldi	r16, 255		; sqr = 1
...
DCOB_TRI_SAW:
	sbrs	r31, 2			; b2 = OSC B wave: 0=normal SAW, 1=extended tri
...
; triangle
	...
	lsl	r22
	rol	r16
	sbrs	r31, 3			; b3 = OSC B wave: 0=normal, 1=sin
...
CALC_DIST:
	...
	sbrc	R30, 0			; b0 = DCO DIST: 0=off, 1=on
	eor	R17, R16
	sbrs	R30, 3			; osc B: 0=off, 1=on
...
RINGMOD:
	subi	r17, $80		; DCO A wave -127..127
	asr	r17			; make a little bit headroom
	subi	r16, $80		; DCO B wave -127..127
	sts	WAVEB,r16		; store signed DCO B wave for fm
	asr	r16			; make a little bit headroom
...
; use SCALED_RESONANCE, MODEFLAGS2(3), LPF_I, HPF_I
; a_H, a_L, b_H, b_L is set to zero in MLP when a pressed key is detected
...
; R17:16 = HDAC:LDAC  the sample output register
...
;initialize sound parameters:
	...
	ldi	R16, 0x14		;\ LFO = DCF, DCF KBD TRACK = 1
	sts	MODEFLAGS2, R16		;/ Knob shift = upper
...
; print knob parameter values from eeprom to serial port
	...
	lds	r16, DETUNEB_FRAC
	call	usart_write_byte

	lds	r16, DETUNEB_INTG
	call	usart_write_byte
...
;-------------------------------------------------------------------------------------------------------------------
	...
	lds	R16, ADC_0
	sbrc	R18, 0			; Check KNOB_STATUS bit 0
...
.if USE_ORIGINAL_MIXER == 1	;(2)
	cpi	R17, $20
	BRLO	SET_DCOA		; limit to 31 if knob is right of the middle
	ldi	R17, $20
.else
	cpi	R17, $02
...
.if USE_ORIGINAL_MIXER == 1	;(2)
	cpi	R16, $20
	BRLO	SET_DCOB		; limit to 31 if knob is left of the middle
	ldi	R16, $20
.else
	cpi	R16, $2
...
.endif
SET_DCOB:
	sts	DCOB_LEVEL, R16		; Store DCOB level (0..31) or (0..63)
...
LOAD_ADC_30:
	...
	sts	SUSTAINLEVEL2,r16	; AD3.0 --> Scaled sustain level
...
.else
;	ldi	R16, 0			; don't use velocity from keyboard
;	sts	VCFENVMOD, R16		; MIDI Velocity --> DCF ENV MOD
.endif

.if MEEBLIP_SE_ENHANCEMENTS == 0
;-------------------------------------------------------------------------------------------------------------------
...
;determine Amin i Amax:
	...
	brne	MLP_LFOAWR		; branch on LFO==squ
	tst	R31			; test flag
	breq	MLP_LFOAX		; branch on not over- or underflow
	lds	R16, SHIFTREG_0		;\
...
;determine Amin i Amax:
	...
	andi	R30, 0xC0
	cpi	R30, 0x80		; Z = ((LFO==tri)&&(LFO==rand))
	brne	MLP_LFO2AWR		; branch on LFO==squ
...
MLP_LFO2AX:
	lds	R16, MODEFLAGS4
	andi	R16, 0xC0
	cpi	R16, 0xC0		; Z = ((LFO==squ)&&(LFO==rand))
	brne	MLP_LFO2NORM
	tst	R31			; flag == 1, branch on not over- or underflow
...
;switch tri/squ:
	lds	R16, MODEFLAGS4		;\ Z=0: triangle
	andi	R16, 0x40		;/ Z=1: square
	breq	MLP_LFO2WR
	lsl	R21			; Cy = (LFO < 0)
	ldi	R21, 127		;\
	adc	R21, ZERO		;/ R21 = -128 or +127
...
;(1)	lds	R21, LFO2VALUE
	lds	R16, MODEFLAGS2
	andi	R16, 0x20
	breq	MLP_LFO2MWX		; skip when MOD.WHEEL = off
	lds	R16, PANEL_LFOLEVEL
	lds	R17, MIDIMODWHEEL
	cp	R16, R17
	brsh	MLP_LFO2LWR
...
; LFO2LEVEL	is LFO2VALUE		when MOD.WHEEL is off
;		is PANEL_LFOLEVEL	when MOD.WHEEL is on and PANEL_LFOLEVEL is greater MOD.WHEEL
;		is MIDIMODWHEEL		when MOD.WHEEL is on and PANEL_LFOLEVEL is lower MOD.WHEEL

MLP_LFO2MWX:
; -------------

; calculate pwm width
; PULSE_WIDTH is independent from PANEL_LFOLEVEL and  MIDIMODWHEEL
...
.else
	...
	mov	r30, r1
	ldi	R31, 0			; \
	sbc	r31, r31		; / sign extension
.endif
					; R31:R30 = -255..+255
MLP_DCF0:

;ENV mod:
.if USE_ORIGINAL_VCFENVMOD == 1
...
startup_message:
	...
	.db	"avrSynth 2.0 (c)AWe 2011", 0x0d, 0x0a
	.db	"   build ", __DATE__, " ", __TIME__, 0x0d, 0x0a, 0
.endif
